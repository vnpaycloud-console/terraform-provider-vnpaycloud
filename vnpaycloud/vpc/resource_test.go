...
func TestResourceVpcCreate_WithoutCIDRAutoGenerated(t *testing.T) {
	vpc := testVPC()
	vpc.Name = "test-auto-cidr-vpc"
	vpc.Description = "auto cidr test"
	...
	srv := testhelpers.NewMockServer(t, []testhelpers.Route{
		{
			...
			Handler: func(w http.ResponseWriter, r *http.Request) {
				...
				if err != nil {
					t.Fatalf("failed to read create request: %v", err)
				}
				if bytes.Contains(body, []byte(`"cidr"`)) {
					t.Errorf("expected cidr field to be omitted so backend can auto-generate, got body %s", body)
				}
				...
				if req.Name != "test-auto-cidr-vpc" {
					t.Errorf("expected name test-auto-cidr-vpc, got %s", req.Name)
				}
				if req.Description != "auto cidr test" {
					t.Errorf("expected description auto cidr test, got %s", req.Description)
				}
				if req.CIDR != "" {
					t.Errorf("expected empty cidr so backend can auto-generate, got %s", req.CIDR)
				}
	...
	res := ResourceVpc()
	d := schema.TestResourceDataRaw(t, res.Schema, map[string]interface{}{
		"name":        "test-auto-cidr-vpc",
		"description": "auto cidr test",
	})
	...
	if !postCalled {
		t.Fatal("expected VPC create request to be sent")
	}
	if v := d.Get("cidr").(string); v != "10.42.0.0/16" {
		t.Errorf("expected generated cidr 10.42.0.0/16, got %s", v)
	}
}

func TestResourceVpcCreate_ReadsSNATFields(t *testing.T) {
	vpc := testVPC()
	vpc.EnableSnat = true
