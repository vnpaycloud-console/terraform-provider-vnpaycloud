---
page_title: "vnpaycloud_database_redis_account Resource - VNPayCloud"
subcategory: "Database"
description: |-
  Manages an account (ACL user) on a VNPayCloud DBaaS Redis instance.
---

# vnpaycloud_database_redis_account (Resource)

Manages an account (Redis ACL user) on a VNPayCloud DBaaS Redis instance.
...
```hcl
resource "vnpaycloud_database_redis_account" "app" {
  name               = "appuser"
  redis_instance_id  = vnpaycloud_database_redis_instance.redis.id
...
- `redis_instance_id` (String, Forces new resource) — ID of the Redis instance this account belongs to.
...
terraform import vnpaycloud_database_redis_account.app <account-id>
```
